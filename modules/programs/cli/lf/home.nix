        ...
        extraConfig = let
          previewer = pkgs.writeShellScriptBin "pv.sh" ''
            ...
            if [[ "$( ${getExe pkgs.file} -Lb --mime-type "$file")" =~ ^image ]]; then
              ${getExe' pkgs.kitty "kitty"} +kitten icat --silent --stdin no --transfer-mode file --place "''${w}x''${h}@''${x}x''${y}" "$file" < /dev/null > /dev/null 2>&1
            elif [[ "$( ${getExe pkgs.file} -Lb --mime-type "$file")" =~ ^text ]]; then
              ${getExe pkgs.bat} --color=always --paging=never --style=numbers "$file"
            else
