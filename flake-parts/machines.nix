{
  ...
  inputs,
  machineConfig,
  kbdLayout, # Add kbdLayout to the function arguments
  kbdVariant, # Add kbdVariant to the function arguments
  ...
}:
with lib; {
  ...
  # Allow broken packages for pgvector
  ...
  programs.nh = mkIf (machineConfig.role != "server") {
    enable = true;
    clean = {
      enable = false; # Disabled to avoid conflict with nix.gc.automatic
  ...
  services.xserver = mkIf (machineConfig.desktop != null) {
    xkb = {
      layout = kbdLayout; # Use kbdLayout directly
      variant = kbdVariant; # Use kbdVariant directly
    };
  };
  ...
  boot = {
    ...
    loader = {
      ...
      grub = mkIf (machineConfig.role != "server") {
        ...
        # Theme handled by Stylix for consistent theming
      };
